...
public class FoursquareModule implements ModuleInterface {
    ...
    public WithinSiteServiceModuleResult getNearestUninformedInterestingPlaces(double x, double y) {
        ...
        try {
            ...
            Date basicQueryReturned = new Date();
            ...
            Date endIndividualVenueInterations = new Date();

            logger.info("STATS: Basic Query: " + ((basicQueryReturned.getTime()-queryStart.getTime())/1000) +
                    "All Venue iterations Query: "+ ((endIndividualVenueInterations.getTime()-basicQueryReturned.getTime())/1000));

        } catch (Exception e) {
            logger.warn("Error populating Foursquare queries to WithinSiteServiceModuleResult places.");
            e.printStackTrace();
        }
    ...
    public WithinSitePlace getPlaceForServiceId(String serviceId, WithinSiteUser user) {
        ...
        try {
            CompleteVenue completeVenue = foursquareApi.venue(serviceId).getResult();
